...
int server(int port) {
	...
	// Binding the socket
	if (bind(socketFileDesc, (struct sockaddr *) &serverAddress,
	...
	if (listenValue < 0) {
		cout << "LIIIIIIIIIIIIIIIIIIIIIIIINDA LISTENNNNNNNNNNNNNNNNNNNNNNNN"
				<< endl;
	}
	...
	while ((client_socket = accept(socketFileDesc,
			(struct sockaddr *) &clientAddress, &sizeOfAddrClient))) {

		// Accept clients
		// sizeOfAddrClient is unsigned int. cplucplus example declared it to be socklen_t
		//sizeOfAddrClient = sizeof(clientAddress);
		int newSocketFileDesc = client_socket; //accept(socketFileDesc, (struct sockaddr *) &clientAddress, &sizeOfAddrClient);
		cout << endl;
		cout << "client socket is --------------------------------- "
				<< client_socket << endl;
		cout << "server socket is --------------------------------- "
				<< socketFileDesc << endl;
		...
		cout << "\t\ttestingSS remaining stepping stones: "
				<< testingSS->remainingSS << endl;
		cout << endl;
		
		testingSS->clientFD = newSocketFileDesc;
		cout << "The client FD that was put into the struct: "<< testingSS->clientFD << endl;
		...
		// pthread_create
		vector<pair<string, int>> theList = convertStringToList(
				testingSS->SSList);

		//cout << "Starting creation of thread" << endl;
		pthread_create(&cThreads, &attr, unpackForNextSteppingStoneConnection,
				(void *) testingSS);
...
void *unpackForNextSteppingStoneConnection(void *SSInfoArg) {
	...
	cout << "The client socket discriptor after being passed to the thread is: " << ss_data->clientFD << endl;
	cout << "The number of stepping stones is: " << ss_data->remainingSS << endl;
	...
	if (!listr.empty()) {
		...
		cconnect(next.second, const_cast<char*>(next.first.c_str()),
				ss_data->url, listr);


		cout << endl;
		cout << "Finished Calling cconnect" << endl;
		...
		int j = send(ss_data->clientFD, buffer, sizeof(buffer), 0);

		cout << "ss_data->clentFD is: " << ss_data->clientFD << endl;
		
		if (j < 0) {
			cout << "failed to send the cat pic in the IF STATEMENT" << endl;
		}
		
		cout << "The url is: " << ss_data->url << endl;
		...
		strcpy(testPacket->message, sigh);
		cout << "### testPacket->message contains: " << testPacket->message
				<< endl;
		...
		if (j < 0) {
			...
			cout << "failed to send the cat pic in the ELSE STATEMENT" << endl;
		}
	}
}
...
void cconnect(int port, char* hn, char* url, vector<pair<string, int>> ss) {
	...
	if (connect(socketFileDesc, (sockaddr *) &serverAddress,
			sizeof(serverAddress)) < 0) {
	...
	counter++;
	cout << "The number of threads is: " << counter << endl;
	sleep(10);
...
pair<string, int> popRandom(vector<pair<string, int>>& Stones) {
	...
	srand (time(NULL));int elem = rand() % Stones.size();
